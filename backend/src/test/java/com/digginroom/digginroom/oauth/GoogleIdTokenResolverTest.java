...
class GoogleIdTokenResolverTest {
    ...
    void 기간이_지난_ID_TOKEN을_사용하면_예외를_던진다() {
        assertThatThrownBy(() -> ID_TOKEN_RESOLVER.verify(EXPIRED_ID_TOKEN, Provider.GOOGLE))
                .isInstanceOf(InvalidIdTokenException.class);
    }

    @Test
    void 잘못된_길이의_ID_TOKEN은_예외를_던진다() {
        assertThatThrownBy(() -> ID_TOKEN_RESOLVER.verify(SHORT_ID_TOKEN, Provider.GOOGLE))
                .isInstanceOf(InvalidIdTokenException.class);
    }

    @Test
    void 잘못된_ID_TOKEN은_예외를_던진다() {
        assertThatThrownBy(() -> ID_TOKEN_RESOLVER.verify(INVALID_ID_TOKEN, Provider.GOOGLE))
                .isInstanceOf(InvalidIdTokenException.class);
    }
}
